...
class wp_bootstrap_navwalker extends Walker_Nav_Menu {
	...
	public function start_lvl( &$output, $depth = 0, $args = array() ) {
		...
		$dropdown_menu = apply_filters( 'electro_nav_menu_start_lvl', $dropdown_menu, $indent, $args );
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		if ( strcasecmp( $item->attr_title, 'divider' ) == 0 && $depth === 1 ) {
			...
			$class_names = join( ' ', apply_filters( 'electro_nav_menu_css_class', array_filter( $classes ), $item, $args ) );
			...
			$id = apply_filters( 'electro_nav_menu_item_id', 'menu-item-'. $item->ID, $item, $args );
			...
			$atts = apply_filters( 'electro_nav_menu_link_attributes', $atts, $item, $args, $depth );
			...
			$icon = empty( $item->icon ) ? '' : '<i class="' . esc_attr( $item->icon ) . '"></i>';
